...
import {IFraxtalERC4626MintRedeemer} from "../src/interfaces/IFraxtalERC4626MintRedeemer.sol";
...
contract FraxLendYieldTokenHelpersTest is Test {
    ...
    address public helpers_address;
    ...
    function setUp() public {
        ...
        FraxlendLenderHelpers _Helpers = new FraxlendLenderHelpers(
            fraxlend_pair_address
        );
        helpers_address = address(_Helpers);
        assertTrue(
            fraxlend_pair_address != address(0),
            "fraxlend pair deployment failed"
        );
        deal(frxusd_address, user_address, 1000 ether);
        deal(sfrxusd_address, user_address, 1000 ether);
        IFraxlendPair _FraxlendPair = IFraxlendPair(fraxlend_pair_address);
        uint256 user_frxusd_balance = IERC20(frxusd_address).balanceOf(
            user_address
        );
        console.log("Frxusd balance %d", user_frxusd_balance);
    }

    function testPairAccounting() public {
        // deposit sfrxusd
